...
- ``POST /auth/login-form`` (urlencoded) — a **plain HTML form target**: works with zero
  JavaScript. It exists because the login must not depend on anything a browser quirk or
  extension can interfere with (this codebase has the Safari scars to justify it).
  Parsed by hand so we don't grow a multipart dependency for two fields.
...
HTTPS. Header carriers remain for programmatic clients (see ``session_token``).
"""
...
class LoginResponse(BaseModel):
    token: str  # for header-based clients; the browser gets the same token as an HttpOnly cookie
    merchant: MerchantResponse


@auth_router.post("/auth/login", response_model=LoginResponse)
def login(body: LoginRequest, request: Request, response: Response, container: ContainerDep) -> LoginResponse:
...
def logout(
    ...
    """Revoke the presented session (any carrier) and clear the cookie. Idempotent: an
